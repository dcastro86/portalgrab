#!/usr/bin/env bash
# Runs `portalgrab daemon --lazy` in place of the service, then restores the service.
# Prints LAZY-PAUSE-OK when the stream demonstrably pauses while idle, LAZY-WAKE-OK when a grab
# after idle answers fast. The pause check needs a changing screen (play a video): on a static
# screen an active stream costs nothing either, so it cannot tell paused from active and stays silent.
P=$HOME/.local/bin/portalgrab
was=$(systemctl --user is-active portalgrab)
systemctl --user stop portalgrab
$P daemon --lazy >/dev/null 2>&1 &
pid=$!
trap 'kill -TERM $pid 2>/dev/null; wait $pid 2>/dev/null; [ "$was" = active ] && systemctl --user start portalgrab' EXIT
